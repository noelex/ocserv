...
static void update_main_stats(main_server_st * s, struct proc_st *proc, uint64_t auth_failures, uint64_t avg_auth_time)
{
...
int session_close(main_server_st * s, struct proc_st *proc)
{
	...
	update_main_stats(s, proc, msg->secmod_auth_failures, msg->secmod_avg_auth_time);
